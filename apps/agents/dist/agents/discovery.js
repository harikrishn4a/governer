...
async function augmentQuery(intent) {
    const locationHint = intent.location ? ` near ${intent.location}` : "";
    const budgetHint = intent.budget ? ` under SGD ${intent.budget}` : "";
    const constraintHint = intent.constraints && intent.constraints.length > 0
        ? ` (constraints: ${intent.constraints.join(", ")})`
        : "";
    const contextualRaw = `${intent.raw}${locationHint}${budgetHint}${constraintHint}`;
    logger_1.logger.info("discovery:augment — sending to LLM", { contextualRaw });
    const augmented = await (0, llm_1.llmCall)({
    ...
    logger_1.logger.info("discovery:augment — augmented query ready", { augmentedQuery: withVariety });
    return withVariety;
}
async function fallbackDiscover(augmentedQuery, intent) {
    logger_1.logger.info("discovery:fallback — using searchAndContents + LLM parsing");
    const results = await (0, exa_1.exaSearchAndContents)(augmentedQuery, 8);
    if (!results.results || results.results.length === 0) {
        throw new Error("Exa returned no results for query: " + augmentedQuery);
    }
    logger_1.logger.info("discovery:fallback — Exa returned raw pages", {
    ...
    const contentBlocks = results.results
        ...
        .join("\n\n---\n\n");
    logger_1.logger.info("discovery:fallback — parsing with LLM", { sourceCount: results.results.length });
...
async function runDiscovery(intent) {
    logger_1.logger.section("PHASE 2 — EXA DISCOVERY");
    logger_1.logger.info("discovery:start", { raw: intent.raw });
    const augmentedQuery = await augmentQuery(intent);
    logger_1.logger.info("discovery:exa-agent — launching (up to 3 retries on transient errors)");
    const agentOptions = await (0, exa_1.exaAgentWithRetry)(augmentedQuery);
    let options;
    let path;
    if (agentOptions && agentOptions.length >= 3) {
        ...
        path = "fallback";
        logger_1.logger.warn("discovery — all Exa Agent attempts failed, falling back to searchAndContents + LLM");
        options = await fallbackDiscover(augmentedQuery, intent);
    }
    logger_1.logger.info("discovery:path-used", { path, count: options.length });
    const paymentIntents = options.map(toPaymentIntent);
    logger_1.logger.info("discovery:options-discovered", {
