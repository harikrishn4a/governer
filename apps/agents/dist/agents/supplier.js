...
exports.runSupplierAgent = runSupplierAgent;
exports.runSupplierAgents = runSupplierAgents;
...
function buildSystemPrompt(option, intent, llmLabel) {
...
async function runSupplierAgent(option, intent, supplierIndex) {
    const llm = SUPPLIER_LLMS[supplierIndex % SUPPLIER_LLMS.length];
    logger_1.logger.info(`supplier[${supplierIndex}]:start`, {
        vendor: option.vendor,
        item: option.item,
        price: `SGD ${option.price}`,
        llm: llm.id,
    });
    const result = await (0, llm_1.llmCallJSON)({
        model: llm.id,
    ...
    logger_1.logger.info(`supplier[${supplierIndex}]:complete`, {
        vendor: option.vendor,
        item: option.item,
        fitScore,
        llm: llm.id,
...
async function runSupplierAgents(options, intent) {
    const candidates = options.slice(0, 3);
    logger_1.logger.section("PHASE 3 — SUPPLIER PITCH AGENTS");
    logger_1.logger.info("supplier:fan-out — launching parallel pitch agents", {
        agentCount: candidates.length,
        vendors: candidates.map((o, i) => ({ index: i, vendor: o.vendor, item: o.item, price: `SGD ${o.price}`, llm: SUPPLIER_LLMS[i % SUPPLIER_LLMS.length].id })),
    });
    const pitches = await Promise.all(candidates.map((opt, i) => runSupplierAgent(opt, intent, i)));
    const sorted = [...pitches].sort((a, b) => b.fitScore - a.fitScore);
    logger_1.logger.info("supplier:fan-out-complete — all pitches received", {
