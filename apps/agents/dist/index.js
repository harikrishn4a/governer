...
app.post("/run", async (req, res) => {
    const { intent, contractId, mode, txId: incomingTxId } = req.body;
    ...
    const txId = incomingTxId?.trim() || (0, crypto_1.randomUUID)();
    const emit = (0, event_bus_1.createEmitter)(txId);
    // Ergonomic wrapper for this handler's own emits (procurement events are
    // emitted inside runFullProcurement via the same `emit`).
    const fire = (type, agent, data) => emit({ type, agent, data, timestamp: new Date().toISOString() });
    ...
    try {
        ...
        const { intent: parsedIntent, options, pitches, winner } = await (0, procurement_1.runFullProcurement)(intent.trim(), emit);
