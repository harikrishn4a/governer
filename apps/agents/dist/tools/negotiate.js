...
exports.callNegotiationAPI = callNegotiationAPI;
...
async function callOnce(payload, attempt) {
    logger_1.logger.info("negotiate:http-request", {
        attempt,
        url: NEGOTIATE_URL,
        method: "POST",
    ...
    logger_1.logger.info("negotiate:http-response", {
        attempt,
        status: res.status,
...
async function callNegotiationAPI(userIntent, options) {
    const payload = {
        itemDescription: userIntent.raw,
        price: userIntent.budget ?? 999,
        options: { options },
    };
    logger_1.logger.info("negotiate:start", {
        url: NEGOTIATE_URL,
        itemDescription: userIntent.raw,
    ...
    try {
        ...
        logger_1.logger.warn("negotiate:attempt-1-failed — retrying", {
    ...
    logger_1.logger.info("negotiate:response-received", {
        elapsedMs,
        resultCount: data.results.length,
        verdict: data.verdict,
    });
    // Log the full ranked list
    logger_1.logger.info("negotiate:full-ranking", {
    ...
    logger_1.logger.info("negotiate:winner-selected", {
        rank: 1,
