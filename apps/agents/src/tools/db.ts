...
export interface TransactionRecord {
  ...
  winner_item: string;
  winner_price: number;
  governance_decision: "ACCEPT" | "BLOCK";
  rationale: string;
  checked_rules: Array<{ rule: string; passed: boolean; detail: string }>;
...
export async function getAllContracts(): Promise<SpendingContract[]> {
  const { rows } = await getPool().query(
    `SELECT id, name, budget_cap, budget_period, category_constraints,
...
export async function getActiveContract(contractId?: string): Promise<SpendingContract> {
  const q = contractId
  ...
  if (!rows[0]) throw new Error("No active spending contract found");
  return rowToContract(rows[0]);
}

export async function createContract(c: Omit<SpendingContract, "id" | "createdAt">): Promise<SpendingContract> {
  const { rows } = await getPool().query(
    ...
    [c.name, c.budgetCap, c.budgetPeriod, c.categoryConstraints, c.vendorBlocklist,
     c.vendorAllowlist, c.riskThreshold, c.active]
  );
  return rowToContract(rows[0]);
}

export async function updateContract(id: string, patch: Partial<Omit<SpendingContract, "id" | "createdAt">>): Promise<SpendingContract> {
  const fields: string[] = [];
  const values: unknown[] = [];
  let i = 1;
  if (patch.name !== undefined)               { fields.push(`name=$${i++}`);                values.push(patch.name); }
  if (patch.budgetCap !== undefined)          { fields.push(`budget_cap=$${i++}`);          values.push(patch.budgetCap); }
  if (patch.budgetPeriod !== undefined)       { fields.push(`budget_period=$${i++}`);       values.push(patch.budgetPeriod); }
  if (patch.categoryConstraints !== undefined){ fields.push(`category_constraints=$${i++}`);values.push(patch.categoryConstraints); }
  if (patch.vendorBlocklist !== undefined)    { fields.push(`vendor_blocklist=$${i++}`);    values.push(patch.vendorBlocklist); }
  if (patch.vendorAllowlist !== undefined)    { fields.push(`vendor_allowlist=$${i++}`);    values.push(patch.vendorAllowlist); }
  if (patch.riskThreshold !== undefined)      { fields.push(`risk_threshold=$${i++}`);      values.push(patch.riskThreshold); }
  if (patch.active !== undefined)             { fields.push(`active=$${i++}`);              values.push(patch.active); }
  values.push(id);
  const { rows } = await getPool().query(
    `UPDATE spending_contracts SET ${fields.join(",")} WHERE id=$${i} RETURNING *`, values
  );
  return rowToContract(rows[0]);
}
...
export async function getTransactionsByContract(contractId: string): Promise<TransactionRecord[]> {
  const { rows } = await getPool().query(
...
export async function overrideTransaction(
  id: string, approvedBy: string, stripeStatus: string
): Promise<void> {
  await getPool().query(
    `UPDATE transactions SET overridden_by=$1, overridden_at=NOW(), stripe_status=$2 WHERE id=$3`,
    [approvedBy, stripeStatus, id]
  );
}

// ── Audit events ────────────────────────────────────────────────────────────

export async function saveAuditEvent(event: AuditEventRecord): Promise<void> {
  await getPool().query(
    `INSERT INTO audit_events (transaction_id, contract_id, event_type, detail, actor)
     VALUES ($1,$2,$3,$4,$5)`,
    [event.transaction_id ?? null, event.contract_id ?? null,
...
function rowToContract(r: any): SpendingContract {
  return {
    id: r.id,
    name: r.name,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToTransaction(r: any): TransactionRecord {
  return {
    id: r.id,
